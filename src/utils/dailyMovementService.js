...
export const upsertDailyMovement = async ({
  ...
  const next = {
    ...
    movement_date: date,
    opening_qty: Number(existing?.opening_qty ?? openingRecord?.opening_qty ?? 0),
    ...
    closing_qty: Number(existing?.closing_qty ?? existing?.opening_qty ?? openingRecord?.opening_qty ?? 0),
  };
